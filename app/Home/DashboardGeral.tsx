...
type Compra = {
  id: string; cartaoId: string; nome: string; valor: number;
...
const monthLabelPt = (d: Date) =>
  d.toLocaleString('pt-BR', { month: 'short' }).replace('.', '').replace(/^./, (ch) => ch.toUpperCase());
...
export default function DashboardGeral() {
  ...
  useEffect(() => {
    ...
    const unsub1 = onSnapshot(
      query(collection(db, 'users', uid, 'rendas_fixas'), orderBy('diaVencimento', 'asc')),
      (snap) =>
        setRendas(
          snap.docs.map((d) => {
            const x = d.data() as Record<string, unknown>;
            return { id: d.id, valor: getNum(x.valor), createdAt: x.createdAt };
          }),
        ),
    );

    const unsub2 = onSnapshot(
      query(collection(db, 'users', uid, 'gastos_fixos'), where('ativo', '==', true), orderBy('diaVencimento', 'asc')),
      (snap) =>
        setGastosFixos(
          snap.docs.map((d) => {
            const x = d.data() as Record<string, unknown>;
            return { id: d.id, valor: getNum(x.valor), ativo: getBool(x.ativo, true), createdAt: x.createdAt };
          }),
        ),
    );

    const unsub3 = onSnapshot(
      query(collection(db, 'users', uid, 'cartoes'), orderBy('nome', 'asc')),
      (snap) =>
        setCartoes(
          snap.docs.map((d) => {
            const x = d.data() as Record<string, unknown>;
            return {
              id: d.id,
              nome: getStr(x.nome),
              cor: getStr(x.cor, '#3b82f6'),
              limite: getNum(x.limite),
            };
          }),
        ),
    );
    ...
    const unsub4 = onSnapshot(
      query(collection(db, 'users', uid, 'cartao_compras'), orderBy('dataISO', 'asc')),
      (snap) =>
        setCompras(
          snap.docs.map((d) => {
            const x = d.data() as Record<string, unknown>;
            return {
              id: d.id,
              cartaoId: getStr(x.cartaoId),
              nome: getStr(x.nome),
              valor: getNum(x.valor),
              parcelas: Math.max(1, getNum(x.parcelas, 1)),
              parcelaAtual: Math.max(1, getNum(x.parcelaAtual, 1)),
              dataISO: getStr(x.dataISO, new Date().toISOString().slice(0, 10)),
    ...
    const unsub5 = onSnapshot(
      query(collection(db, 'users', uid, 'poupanca_transacoes'), orderBy('createdAt', 'asc')),
      (snap) =>
        setPoupTx(
          snap.docs.map((d) => {
            const x = d.data() as Record<string, unknown>;
            const tipo = getStr(x.tipo) === 'retirada' ? 'retirada' : 'deposito';
            return { id: d.id, tipo, valor: getNum(x.valor), createdAt: x.createdAt };
          }),
        ),
    );
  ...
  }, [authReady, authError, uid]);

  /* ---------------- Agregações ---------------- */
  const totalRendaFixaMes = useMemo(
    () => rendas.reduce((a, r) => a + getNum(r.valor), 0),
    [rendas],
  );

  const totalGastosFixosMes = useMemo(
    () => gastosFixos.reduce((a, g) => a + getNum(g.valor), 0),
    [gastosFixos],
  );

  const gastoCartoesMes = useMemo(
    () => compras.reduce((a, c) => a + getNum(c.valor) / Math.max(1, getNum(c.parcelas, 1)), 0),
    [compras],
  );

  const poupancaDepositadoMes = useMemo(
    () => poupTx.filter((t) => t.tipo === 'deposito').reduce((a, t) => a + getNum(t.valor), 0),
    [poupTx],
  );
  const poupancaRetiradoMes = useMemo(
    () => poupTx.filter((t) => t.tipo === 'retirada').reduce((a, t) => a + getNum(t.valor), 0),
    [poupTx],
  );
  ...
  const buckets = useMemo(() => {
    // base de data local ao memo (não é ref, não quebra o linter)
    const base = new Date();
    ...
    // renda fixa e gastos fixos: constantes por mês
    for (const b of arr) b.ganhos = totalRendaFixaMes;
    for (const b of arr) b.gastos += totalGastosFixosMes;

    // compras de cartão: soma por parcela do mês de compra
    ...
    return arr;
  }, [totalRendaFixaMes, totalGastosFixosMes, compras]);
  ...
  const saldoTotalAproximado = useMemo(() => saldoMes + poupancaSaldoMes, [saldoMes, poupancaSaldoMes]);

  if (!authReady) return null;
  if (authError) {
    return (
      <div className="p-6 text-sm text-red-600">
        Erro de autenticação: {authError}. Faça login para visualizar o painel.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* HERO */}
      <Card className="bg-linear-to-br from-blue-600 via-blue-500 to-cyan-500 text-white border-0 shadow-xl">
          ...
          return (
            <Card key={i} className="dark:bg-slate-800/50 dark:border-slate-700">
              <CardContent className="p-6">
                ...
                <p className="text-slate-900">R$ {c.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
              </CardContent>
            </Card>
          );
        })}
      ...
      {/* GANHOS x GASTOS e DISTRIBUIÇÃO */}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart
              data={buckets.map((b, idx) => ({
                mes: b.label,
                saldo: (b.ganhos - b.gastos) + (idx === buckets.length - 1 ? poupancaSaldoMes : 0),
              }))}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="mes" stroke="#64748b" />
